...
class BFake:
    ...
    def rand_time_spent(self):
        if (self.LOGNORM_TIME_SPENT):
    ...
    def get_rand_URL(self):
        ...
        while ((url_suffix != "com") and 
               (url_suffix != "net") and
               (url_suffix != "org")):
            random_index = int(random.triangular(0, len(self.urls_list)-1, 0))
            url = self.urls_list[random_index].strip()
            url_split = url.split('.')
    ...
    def main(self):
        ...
        for line in csvreader:
            self.urls_list.append("http://" + line[1]) 
        sel_port = int(sys.argv[2])
        # Begin
        print "Read in %d websites from %s" % (len(self.urls_list), sys.argv[1])
        current_url = self.get_rand_URL()
        print "current_url = %s" % (current_url)
        #Set up server
        sel = selenium("localhost", sel_port, "*firefox", current_url)
        for attempts in range(self.MAX_ATTEMPTS_SERVERCONNECT):
            try:
                sel.start()
                break
            except Exception as e:
                print e
                if (attempts > self.MAX_ATTEMPTS_SERVERCONNECT-2):
                    print "Failed after %s attempts" % (self.MAX_ATTEMPTS_SERVERCONNECT)
                    sel.stop()
                    return 1
                else:
